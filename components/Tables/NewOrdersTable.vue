      ...
      <div v-if="tableData.length">
        <div class="mobile_no_show table-container come-down">
          <table>
            ...
            <tr v-for="(data, index) in filteredTable" :key="index" class="table-details">
              <td class="date">
                {{ detailedDate(data.collectionDate) }}
              </td>
              <td class="time">
                {{ data.collectionTime }}
              </td>
              ...
              <td class="action-ctn">
                <div class="action">
                  <svg
                    ...
                    @click="$router.push(`/${$route.name}/new-orders?id=${data.id}`)"
                  >
        ...
        <div class="no_show">
          ...
          <TablesFooter
            ...
            @prev="$emit('prev')"
          />
          <div v-for="(data, index) in filteredTable" :key="index" class="data_box slide-in-from-left" @click="$router.push(`/${$route.name}/new-orders?id=${data.id}`)">
            <div>
              <p class="mobile_time">
                {{ data.collectionTime }}
              </p>
...
export default {
  ...
  computed: {
    filteredTable () {
      return this.tableData.filter(data => this.detailedDate(data.collectionDate).toLowerCase().includes(this.tableQuery.toLowerCase()) || data.collectionTime.toLowerCase().includes(this.tableQuery.toLowerCase()) || data.cAddress.toLowerCase().includes(this.tableQuery.toLowerCase()))
    }
  },
  methods: {
  }
