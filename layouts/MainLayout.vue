  ...
  methods: {
    ...
    getUserDetails () {
      this.$axios.$get('/auth/all/registration/information', {
        ...
        if (response.error && (response.errorMsg === 'Authentication Failed' || response.errorMsg === 'Unauthorized User' || response.errorMsg === 'invalid signature' || response.errorMsg === 'jwt malformed')) {
          this.$router.push('/auth/login?error=session has expired')
        } else if (
